...
class AnimationBatchProcessor {
    ...
    func processBatch(_ animations: [BatchedAnimation]) async -> [Rect] {
        ...
        case .gpu(let device, let pipeline):
            results = await processGPUBatch(batch, device: device, pipeline: pipeline)
        case .cpu:
            results = await processCPUBatch(batch)
        case .hybrid(let gpuCount, let cpuCount):
            results = await processHybridBatch(batch, gpuCount: gpuCount, cpuCount: cpuCount)
        }
    ...
    private func performGPUInterpolation(_ animations: [BatchedAnimation], device: MTLDevice, pipeline: MTLComputePipelineState, commandQueue: MTLCommandQueue) async throws -> [Rect] {
        ...
        guard let sourceBuffer = device.makeBuffer(length: bufferSize, options: .storageModeShared),
              ...
              let paramsBuffer = device.makeBuffer(length: MemoryLayout<InterpolationParams>.stride, options: .storageModeShared) else {
    ...
    nonisolated private func interpolateRectCPU(from source: Rect, to target: Rect, progress: Double, easing: EasingType) -> Rect {
    ...
    nonisolated private func applyCPUEasing(_ progress: Double, type: EasingType) -> Double {
        switch type {
        case .linear:
            return progress
        case .easeIn:
    ...
    private func recordBatchProcessingTime(_ time: TimeInterval, mode: ProcessingMode) {
        ...
        case .cpu:
            modeDescription = "CPU"
        case .hybrid(let gpuCount, let cpuCount):
            modeDescription = "Hybrid(GPU:\(gpuCount), CPU:\(cpuCount))"
        }
