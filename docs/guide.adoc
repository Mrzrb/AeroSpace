...
BSP (Binary Space Partitioning) is a layout that recursively splits space into a binary tree structure, similar to i3wm. When you add a new window, BSP intelligently chooses where to split existing space based on container dimensions and configuration.

* *Horizontal BSP* (`h_bsp`) prefers horizontal splits when adding new windows
* *Vertical BSP* (`v_bsp`) prefers vertical splits when adding new windows  
* *Auto BSP* (`bsp`) intelligently chooses split direction based on container aspect ratio

BSP layout creates a more flexible window arrangement compared to traditional tiles, allowing for complex nested layouts while maintaining intuitive navigation with the xref:commands.adoc#focus[focus] command.
